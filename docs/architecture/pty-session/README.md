...
4. ~~**Daemon binary** — bind the UDS listener, run the wRPC `Handler` impls (meta + all unary session ops + streaming attach/logs/send/wait) against the session registry, route inbound `sessions.attach` streams to per-session command channels ([[daemon-process-model]], [[internal-communication]]).~~ **Done (UDS, v0).** See spec `docs/superpowers/specs/2026-05-26-daemon-binary-design.md` and plans `docs/superpowers/plans/2026-05-26-daemon-core.md` (meta + unary ops), `docs/superpowers/plans/2026-05-26-daemon-streaming.md` (attach/logs/send/wait). **Deferred (future work):** WebTransport listener + bearer-token first-message auth + TLS; idle-timeout enforcement + exited-session reaping; max-concurrent-sessions cap; time-windowed (`since`) `logs` (needs a timestamped transcript); Prometheus `/metrics` + `/debug/sessions` endpoint; TOML config loader.
