...
9. **Observability** — `tracing` subscriber installation, per-session spans, debug endpoint ([[observability]]). **Partial:** `tracing_subscriber::fmt` with `EnvFilter` installed (configurable via `CAIRN_LOG` / `--log`); per-session spans, structured lifecycle events, Prometheus `/metrics`, and `/debug/sessions` endpoint not yet done.
