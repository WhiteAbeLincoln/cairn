...
4. ~~**Daemon binary** — bind the UDS listener, run the wRPC `Handler` impls (meta + all unary session ops + streaming attach/logs/send/wait) against the session registry, route inbound `sessions.attach` streams to per-session command channels ([[daemon-process-model]], [[internal-communication]]).~~ **Done (UDS, v0).** See spec `docs/superpowers/specs/2026-05-26-daemon-binary-design.md` and plans `docs/superpowers/plans/2026-05-26-daemon-core.md` (meta + unary ops), `docs/superpowers/plans/2026-05-26-daemon-streaming.md` (attach/logs/send/wait). **Deferred (future work):** idle-timeout enforcement + exited-session reaping; max-concurrent-sessions cap; time-windowed (`since`) `logs` (needs a timestamped transcript); Prometheus `/metrics` + `/debug/sessions` endpoint; TOML config loader. (WebTransport listener + bearer-token first-message auth + TLS are now done — see items 6 and 7.)
5. ~~**Wire protocol** — wRPC over a `cairn:daemon@0.1.0` WIT schema; transports are Unix Domain Socket (local) and WebTransport (remote, browser and CLI).~~ **Done.** See spec `docs/superpowers/specs/2026-05-26-daemon-protocol-design.md` and crate `crates/cairn-protocol/`. (Daemon binary wiring done — item 4. Auth done — item 6. Observability partially done — item 9.)
6. ~~**Authentication** — bearer-token + Origin checks; Unix socket fallback ([[authentication]]).~~ **Done.** Bearer token via first-message `meta.authenticate` RPC with constant-time comparison; SO_PEERCRED on UDS; pluggable `AuthBackend` trait with `AuthChain`; WebTransport + TLS with ECDSA P-256 cert generation and SPKI hash pinning; Tailscale LocalAPI whois backend. Origin checks replaced by loopback-only default bind + token defense.
7. ~~**CLI client binary** — termios raw mode, Ctrl+\ detach detection, SIGWINCH propagation, signal forwarding ([[web-vs-cli-clients]]).~~ **Done.** `cairn-client` crate with `RawGuard` RAII termios, docker-style detach-key matching (with Kitty CSI-u support + 50ms escape timeout), SIGWINCH → resize forwarding, SIGINT/SIGTERM/SIGQUIT/SIGHUP graceful detach. 14+ subcommands: exec, run, attach, list, inspect, send, kill, wait, logs, kick, restart, rename, whoami, version, completion. Both UDS and WebTransport transports.
8. **Backpressure policy** — per-client transport backpressure via `Sink::poll_ready`, lag → close → reconnect-with-snapshot ([[backpressure]]). **Partial:** `Lagged` detection + client eviction with `client.lagged` error code + reconnect-with-snapshot implemented; per-client transport-level `Sink::poll_ready` backpressure and tuning knobs not yet done.
9. ~~**Observability** — `tracing` subscriber installation, per-session spans, debug endpoint ([[observability]]).~~ **Done (v0).** Configurable log format (pretty/compact/json/full/off) via `--log-format`; OTLP trace layer gated on `OTEL_EXPORTER_OTLP_ENDPOINT`; per-session `pty_session` spans; per-command `cmd` spans with OTel span links across the flume boundary; per-RPC `rpc` spans with W3C traceparent span links; per-attach `attach` spans with lifecycle events; `call-context` record on all WIT operations. **Deferred:** CLI-side OTLP export; OTLP push metrics.
10. ~~**Daemon-level tests** — subprocess + in-process WS client harness ([[testing]]).~~ **Done.** `DaemonHarness` (in-process, tempdir UDS + optional WebTransport) with 26 daemon integration tests across unary ops, streaming, meta, and WebTransport; subprocess smoke test of real `cairn-daemon` binary; 24 client integration tests in `cairn-client/tests/`.
...
| [[observability]] | Logging, tracing, metrics, debug endpoint | Layered subscriber with configurable fmt + OTLP; per-session/RPC/attach spans; `call-context` propagation |
