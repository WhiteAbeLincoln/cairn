...
4. **Daemon binary** — bind the UDS and WebTransport listeners, run the wRPC `Handler` impls against the session registry, route inbound `sessions.attach` streams to per-session command channels ([[daemon-process-model]], [[internal-communication]]). **Designed (UDS, v0).** See spec `docs/superpowers/specs/2026-05-26-daemon-binary-design.md`. v0 serves **UDS only**; the WebTransport listener + token auth is a follow-up. Implementation pending. **Deferred from daemon v0 (future work):** idle-timeout enforcement + exited-session reaping; max-concurrent-sessions cap; time-windowed (`since`) `logs` (needs a timestamped transcript); WebTransport transport + bearer-token first-message auth + TLS; Prometheus `/metrics` + `/debug/sessions` endpoint; TOML config loader.
